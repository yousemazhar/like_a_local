...
class _AllPlacesList extends ConsumerWidget {
  const _AllPlacesList();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final placesAsync = ref.watch(discoverFeedProvider);
    return SizedBox(
      height: 184,
      child: placesAsync.when(
        loading: () => ListView.builder(
          ...
          itemBuilder: (_, __) => const Padding(
            ...
            child: SkeletonCard(height: 52),
          ),
        ),
        error: (_, __) => _AllPlacesFallback(),
        data: (places) {
          final list = places.isNotEmpty ? places : const <Place>[];
          if (list.isEmpty) return _AllPlacesFallback();
...
class _AllPlacesFallback extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return ListView.builder(
      padding: EdgeInsets.zero,
      ...
      itemBuilder: (_, i) => Padding(
        padding: const EdgeInsets.only(bottom: 10),
        child: Text(
          '${_demoPlaces[i].title} · ${_demoPlaces[i].neighborhood}',
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
          style: LALTypography.labelLarge.copyWith(color: Colors.white),
        ),
      ),
    );
  }
