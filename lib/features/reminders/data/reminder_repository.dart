...
class ReminderRepository {
  ...
  Stream<List<Reminder>> stream(String uid) =>
      _items(uid).snapshots().map((s) => s.docs
  ...
  Future<void> set({
    ...
    int radiusMeters = 200,
      ...
      _items(uid).doc(placeId).set({
        ...
        'enabled': true,
        if (placeTitle != null) 'placeTitle': placeTitle,
        'createdAt': FieldValue.serverTimestamp(),
      });

  Future<void> setEnabled(String uid, String placeId, bool enabled) =>
      _items(uid).doc(placeId).set(
        {'enabled': enabled},
